...
# --- Debug: show table columns (to avoid column mismatch)
...
for table in ["providers", "food_listings", "receivers", "claims"]:
    try:
        ...
        st.write(f"{table} table columns:", cols)
    except Exception as e:
        st.write(f"⚠️ Error reading {table}: {e}")

# --- Sidebar Filters (City / Provider Type / Food Type / Meal Type)
st.sidebar.header("Filters")
cities = run_query("SELECT DISTINCT City FROM providers ORDER BY City")["City"].dropna().tolist()
provider_types = run_query("SELECT DISTINCT Type FROM providers ORDER BY Type")["Type"].dropna().tolist()
...
# --- Tabs: Queries | Listings | Claims | Contacts | CRUD
...
with tab3:
    ...
    claim_status = st.multiselect("Filter Claim Status", run_query("SELECT DISTINCT Status FROM claims")["Status"].tolist())
...
with tab4:
    ...
    pc = run_query("""SELECT Name, Type, City, Contact FROM providers
                      {} ORDER BY City, Name""".format(
                        "WHERE City IN (" + ",".join([f":c{i}" for i in range(len(city_f))]) + ")" if city_f else ""
                    ), params=params)
    rc = run_query("""SELECT Name, Type, City, Contact FROM receivers
                      {} ORDER BY City, Name""".format(
                        "WHERE City IN (" + ",".join([f":c{i}" for i in range(len(city_f))]) + ")" if city_f else ""
                    ), params=params)
    c1, c2 = st.columns(2)
